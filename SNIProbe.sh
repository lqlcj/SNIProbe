...
check_reality_dest_domain(){
  local input i d raw_list
  local -a domains
  local summary=""
  local domain ips ip ipn org cdn_match proto x25 pq alpn cipher verdict
  local v4s v6s vps_has_v6 conn hs1 hs2 hs3
  local cert_san cert_match san san_value suffix
  local any_fail fastest t testip idx=0

  echo ""
  read -p "  请输入要测试的域名（支持批量：多个域名用英文逗号分隔，如 https://claude.ai,github.com，回车取消）: " input
  input=$(printf '%s' "$input" | tr -d ' \t\r\n')
  if [ -z "$input" ]; then
    return 0
  fi

  # 按英文逗号拆分为批量列表，逐个剥离协议/路径并做格式校验
  IFS=',' read -r -a raw_list <<< "$input"
  for i in "${raw_list[@]}"; do
    i=$(printf '%s' "$i" | sed -E 's#^[a-zA-Z][a-zA-Z0-9+.-]*://##; s#[/?#].*$##' | tr -d ' ')
    [ -z "$i" ] && continue
    if ! printf '%s' "$i" | grep -qE '^[a-zA-Z0-9._-]+$'; then
      echo -e "${R}  忽略不合法域名：$i（仅允许字母、数字、点、横线、下划线）${N}"
      continue
    fi
    domains+=("$i")
  done
  ...
  if [ ${#domains[@]} -gt 1 ]; then
    echo -e "  ${B}${C}›  SNI 批量检测（共 ${N}${C}${#domains[@]}${N}${B} 个域名）${N}"
  else
    echo -e "  ${B}${C}›  SNI 域名检测: ${N}${domains[0]}"
  fi
  render_divider
  ...
  for d in "${domains[@]}"; do
    domain="$d"
    idx=$((idx + 1))
    ips="" ip="" ipn= org="" cdn_match="" proto= x25= pq= alpn= cipher=
    v4s="" v6s="" vps_has_v6="" conn="" hs1="" hs2="" hs3=
    cert_san="" cert_match=0 san="" san_value="" suffix=
    any_fail=0 fastest=999 t= testip=

    echo ""
    echo -e "  ${B}${C}▪ [$idx/${#domains[@]}] $domain${N}"
    render_divider
    echo ""
    ...
    if [ -z "$ips" ]; then
      echo -e "    ${R}✗ DNS 解析失败${N}"
      verdict="${R}✗ 不可用 — DNS 解析失败${N}"
      summary="${summary}  ${B}${C}$domain${N} → $verdict
"
      continue
    fi
...
EOF
    ...
    # 综合评级
    if [ -n "$cdn_match" ]; then
      verdict="${R}✗ 不可用 — 命中 CDN/云厂商黑名单（$cdn_match）${N}"
    elif [ "$proto" != "TLSv1.3" ]; then
      verdict="${R}✗ 不可用 — 不支持 TLS 1.3（硬性要求）${N}"
    elif [ "$x25" != "yes" ]; then
      verdict="${R}✗ 不可用 — 不支持 X25519 密钥交换（硬性要求）${N}"
    elif [ "$any_fail" = "1" ]; then
      verdict="${Y}△ 慎用 — 有 IP 不通，可能成为定时炸弹（参考 tmu.ac.jp 案例）${N}"
    elif [ "$ipn" -gt 3 ]; then
      verdict="${Y}△ 慎用 — IP 数量较多（${ipn} 个），未来踩死 IP 风险偏高${N}"
    elif [ "$alpn" != "h2" ]; then
      verdict="${Y}△ 可用 — 但 ALPN 是 ${alpn:-未协商}，伪装稍弱${N}"
    elif [ "$cert_match" != "1" ]; then
      verdict="${Y}△ 可用 — 但证书 SAN 未覆盖该域名${N}"
    else
      verdict="${G}✓ 完美 — 全部硬指标通过，建议作为目标站点使用${N}"
    fi
    echo ""
    render_divider
    echo -e "  ${B}综合判定 · $domain${N}"
    echo -e "    $verdict"
    if [ "$fastest" != "999" ]; then
